...
JobSchema.post('save',function(job) {
	...
	var jobBeingSavedStatusId = job.status._id ? job.status._id : job.status;
...
JobSchema.pre('save', function(next) {
	...
	if (!jobBeingSaved.isNew) {
		...
		jobBeingSaved.constructor.findById(jobBeingSaved._id, function (err, currentJob) {
			...
			var jobBeingSavedStatusId = jobBeingSaved.status._id ? jobBeingSaved.status._id : jobBeingSaved.status;
			...
			if (!jobBeingSavedStatusId.equals(currentJob.status)) {
		...
		// and recently created..
	{
		...
		jobBeingSaved.constructor.find(
			{	user: jobBeingSaved.user,
				service_supplier: jobBeingSaved.service_supplier,
				...
				if (err) {
					...
					if(jobs.length) {
						next(new Error('No es posible agregar trabajos para el mismo proveedor y los mismos servicios en el periodo de un mes.'));
					}
					else {
						next();
					}
		...
		function(err, jobs){
			...
			else
			{
				...
				if (jobs.length)
				{
					...
					var completedJobStatusConfig = _.find(config.staticdata.jobStatuses, _.matchesProperty('name', 'Completado'));
