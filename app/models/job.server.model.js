...
/**
 * Testing function...(stores points based on different statuses applied for the job) - not needed now
 *
JobSchema.methods.updatePoints = function(){

	var job = this;
	// Searching for points for the current status - within the job points collection.
	var pointsForStatus = _.find(job.points, _.matchesProperty('status', job.status));

	// If not found, then we'll add them
	if(!pointsForStatus){

		// Getting job status configuration (to get points)
		var job_status_config = _.find(config.staticdata.jobStatuses, _.matchesProperty('_id', job.status));
		if(job_status_config.points){
			this.points.push({status: job.status, points: job_status_config.points});
		}
	}
}*/
